...
function isMeaningful(proposal) {
  ...
  if (proposal.roleRemovalConfig?.subjectId) return true;
  if ((proposal.roleRemovalConfig?.members || []).length > 0) return true;
  ...
  if (proposal.setterCategory) return true;
  if (proposal.electionRoleId) return true;
  if (proposal.roleConfig?.parentHatId) return true;
  return false;
}
